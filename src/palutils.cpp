...
//#define LOG_DEBUG cout
	...
	string getString(std::string key, std::string dflt)
	{
		string x = dflt;
		std::string f, i;
		try
	...
	Dynamic::Var GetJSONObject(Object::Ptr aoJsonObject, const char *aszKey)
	{
		...
		try
		{
			...
			for (i=0; i<ct; i++)
			{
				//LOG_DEBUG << "key: " << tok[i] << endl;
				loVariable = aoJsonObject->get(tok[i]);
				if (i<(ct-1))
				{
					aoJsonObject.reset();
					if (loVariable.size()>0)
					{
						aoJsonObject=loVariable.extract<Object::Ptr>();
					}
					loVariable.clear();
				}
			}
